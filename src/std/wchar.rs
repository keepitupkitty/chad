use {
  crate::{
    ...
    support::{locale, string::wstring_length},
    wchar_t,
    wint_t
  },
  cbitset::BitSet256,
...
pub extern "C" fn ouma_wmemcmp(
  ...
  let mut i = 0;
  while i < n {
    let a = unsafe { *left.wrapping_add(i) };
    let b = unsafe { *right.wrapping_add(i) };
    if a != b {
...
pub extern "C" fn ouma_wcspbrk(
  ...
  unsafe {
    ...
    if *ws1 != 0 { ws1.cast_mut() } else { ptr::null_mut() }
  }
}
...
pub extern "C" fn ouma_wcsstr(
  ...
  unsafe {
    while *wh.wrapping_offset(0_isize) != 0 {
      let mut i = 0;
      loop {
        if *needle.wrapping_offset(i as isize) == 0 {
          return wh.cast_mut();
        }
        if *wh.wrapping_offset(i as isize) !=
          *needle.wrapping_offset(i as isize)
        {
          break;
        }
        i += 1;
...
pub extern "C" fn ouma_mbrtowc(
  ...
  uchar::ouma_mbrtoc32(pwc.cast::<char32_t>(), s, n, ps)
}
...
extern "C" fn ouma_mbsnrtowcs(
  ...
  if dst.is_null() {
    ...
    loop {
      ...
      let l = (loc.ctype.mbtoc32)(&mut c32, sb, nms, ps);
      match l {
        | -1 => {
    ...
    let mut db = dst;
    while i > 0 {
      let l = (loc.ctype.mbtoc32)(db.cast::<char32_t>(), sb, nms, ps);
      match l {
        | -1 => {
          unsafe { *src = sb };
          return -1isize as usize;
...
pub extern "C" fn ouma_wcsnrtombs(
  ...
  if dst.is_null() {
    while srco < nwc {
      let wc: wchar_t = unsafe { *s1.wrapping_add(dsto) };
      ...
      if wc < 0x80 {
        ...
        r = ouma_wcrtomb(buf.as_ptr().cast_mut(), wc, ps);
  ...
  while dsto < len && srco < nwc {
    ...
    if wc < 0x80 {
      unsafe { *dst.wrapping_add(srco) = wc as c_char };
      ...
      string::ouma_memcpy(
        dst.wrapping_add(srco).cast::<c_void>(),
