...
fn mbtoc32(
  ...
  let mut sb: *const c_uchar = s.cast::<c_uchar>();
  let mut i = n;
  if i < 1 {
    return -2;
  }
  ...
  while i > 0 {
    ...
    if bytesleft == 0 {
      if partial < lowerbound ||
        (0xd800..=0xdfff).contains(&partial) ||
        partial > 0x10ffff
      {
        errno::set_errno(errno::EILSEQ);
        return -1;
      ...
      locale::mbstate_set_init(state);
      return unsafe { sb.offset_from(s.cast::<c_uchar>()) };
    }
...
pub const LOCALE_CTYPE_UTF8: locale::ctype::LocaleCtype =
  locale::ctype::LocaleCtype {
    codeset: "UTF-8".as_ptr().cast::<c_char>(),
    mbtoc32,
    c32tomb,
    mb_cur_max: 4
  };
