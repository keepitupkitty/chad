...
extern "C" {
  ...
  extern LocaleStruct OLOCALE_C;
  extern LocaleStruct OLOCALE_C_UTF8;
}

#define olocale_c (&OLOCALE_C)
#define olocale_unicode (&OLOCALE_C_UTF8)
...
TEST(iswgraph, locale) {
  ASSERT_TRUE(ouma_iswgraph_l(L'0', olocale_c));
  ASSERT_TRUE(ouma_iswgraph_l(L'A', olocale_c));
  ASSERT_TRUE(ouma_iswgraph_l(L'a', olocale_c));
  ASSERT_FALSE(ouma_iswgraph_l(L'Å', olocale_c));
  ASSERT_FALSE(ouma_iswgraph_l(L'Ω', olocale_c));
  ASSERT_FALSE(ouma_iswgraph_l(L'д', olocale_c));
  ASSERT_TRUE(ouma_iswgraph_l(L'.', olocale_c));
  ASSERT_FALSE(ouma_iswgraph_l(L'€', olocale_c));
  ASSERT_FALSE(ouma_iswgraph_l(L' ', olocale_c));
}
...
TEST(towlower, locale) {
  EXPECT_EQ(WEOF, ouma_towlower(WEOF));
  EXPECT_EQ(wint_t('!'), ouma_towlower_l(L'!', olocale_c));
  EXPECT_EQ(wint_t('a'), ouma_towlower_l(L'a', olocale_c));
  EXPECT_EQ(wint_t('a'), ouma_towlower_l(L'A', olocale_c));
  EXPECT_EQ(wint_t(L'ç'), ouma_towlower_l(L'ç', olocale_unicode));
  EXPECT_EQ(wint_t(L'ç'), ouma_towlower_l(L'Ç', olocale_unicode));
  EXPECT_EQ(wint_t(L'δ'), ouma_towlower_l(L'δ', olocale_unicode));
  EXPECT_EQ(wint_t(L'δ'), ouma_towlower_l(L'Δ', olocale_unicode));
}
...
TEST(towupper, locale) {
  EXPECT_EQ(WEOF, ouma_towupper_l(WEOF, olocale_c));
  EXPECT_EQ(wint_t('!'), ouma_towupper_l(L'!', olocale_c));
  EXPECT_EQ(wint_t('A'), ouma_towupper_l(L'a', olocale_c));
  EXPECT_EQ(wint_t('A'), ouma_towupper_l(L'A', olocale_c));
  EXPECT_EQ(wint_t(L'Ç'), ouma_towupper_l(L'ç', olocale_unicode));
  EXPECT_EQ(wint_t(L'Ç'), ouma_towupper_l(L'Ç', olocale_unicode));
  EXPECT_EQ(wint_t(L'Δ'), ouma_towupper_l(L'δ', olocale_unicode));
  EXPECT_EQ(wint_t(L'Δ'), ouma_towupper_l(L'Δ', olocale_unicode));
}
